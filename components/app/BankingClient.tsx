...
export function BankingClient({ initialDate }: { initialDate: string }) {
  ...
  async function createLookupAccount(e: React.FormEvent) {
    ...
    const accountNumber = addAccountNumber.trim();
    if (activeLookupType === "GL" && !/^\d{4}$/.test(accountNumber)) {
      toast.error("Account number is required", "Enter a 4 digit account number from the chart.");
      return;
    }
        ...
        <form onSubmit={createLookupAccount} className="space-y-4">
          {activeLookupType === "GL" ? (
            <label className="space-y-2 text-sm">
              <div className="font-bold text-ebony-800">Account Number</div>
              <input
                autoFocus
